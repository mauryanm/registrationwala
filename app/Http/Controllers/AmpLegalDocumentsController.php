...
class AmpLegalDocumentsController extends Controller
{
    public function index()
    {
        $doccat = DocCategory::with('docs:id,doc_category_id,title,heading,slug')->orderBy('id','desc')->get();
        return Voyager::view('amp.legaldocument.index')->with(compact('doccat'));
    }
    public function docpages($url)
    {
        $document = LegalDocument::whereSlug($url)->firstOrFail();
        return Voyager::view('amp.legaldocument.view')->with(compact('document'));
    }
